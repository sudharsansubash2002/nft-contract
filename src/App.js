...
const decimals = async()=>{
	let d = await erc20contract.methods.decimals().call();
		...
		useEffect(()=>{decimals()});	  	
		const approve = async(approveaddress,val2)=>{
			const accounts = await  web3.eth.getAccounts();
			await erc20contract.methods.approve(approveaddress,web3.utils.toBN(val2*10000000000000000000)).send({from:accounts[0]});
		  }

		  function handleapprove() {
			
			console.log(inputadd2.current.value);
			let i=1;
			let h=inputadd2.current.value;
...
return (
	...
	<Card className="text-center">
		...
		<br/>
		<br/>
		<label>Approve</label>&nbsp;&nbsp;<input ref={inputadd2} 
		type="text" 
		id="add2" 
		name="add2" placeholder='enter the address'/>
